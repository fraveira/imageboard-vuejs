...
app.get("/images/:imageId", (req, res) => {
    ...
    db.getPicInfo(id)
        ...
        .catch(err => {
            console.log(
                "Error happened when rendering the PIC with the specific url",
                err
            );
        });
});
...
app.post("/upload", uploader.single("image"), s3.upload, function(req, res) {
    ...
    db.addPictures(username, title, desc, imageUrl)
        .then(function({ rows }) {
            res.json({
                username,
                title,
...
app.get("/moreimages/:lastId", function(req, res) {
    var lastId = req.params.lastId;
    console.log("This is the last id", lastId);
    db.getMoreImages(lastId).then(function({ rows }) {
        res.json(rows);
    });
});
