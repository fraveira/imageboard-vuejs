...
module.exports.getPictures = () => {
    return db.query(`SELECT * FROM images ORDER BY created_at DESC LIMIT 15;`);
};

module.exports.addPictures = (username, title, desc, imageUrl) => {
    return db.query(
        `INSERT INTO images (username, title, description, url) values ($1, $2, $3, $4) RETURNING id;`,
...
module.exports.postingComments = (usercmt, comment, currImg) => {
    return db.query(
        `INSERT INTO comments (comment, usercmt, imageId) values ($1, $2, $3) RETURNING id`,
        [comment, usercmt, currImg]
    );
};
